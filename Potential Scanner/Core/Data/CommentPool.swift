...
//  "2. 전투력의 근거" 문구(16개)를 그대로 옮김. 타입과 완전히 독립적으로 랜덤 추출한다.
//

import Foundation

enum CommentPool {
    /// 코멘트 id: comment.001 ... comment.162
    static let commentIDs: [String] = (1...162).map { String(format: "comment.%03d", $0) }

    /// 클로징 멘트 id: closing.001 ... closing.004
    static let closingLineIDs: [String] = (1...4).map { String(format: "closing.%03d", $0) }

    /// 전투력 산출 근거 문구 id: powerBasis.001 ... powerBasis.016
    static let powerBasisIDs: [String] = (1...16).map { String(format: "powerBasis.%03d", $0) }
